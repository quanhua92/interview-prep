...
graph TD
    subgraph Warp (32 Threads)
        W["Warp Instruction (Load/Store)"] --> LSU["Load/Store Unit (SM)"]
    end
    subgraph Cache Hierarchy
        LSU --> L1["L1 Cache / Shared Memory (128-Byte Lines)"]
        L1 --> L2["L2 Cache (partitioned by memory channels)"]
    end
    subgraph Device Memory
        L2 --> MC["Memory Controllers"]
        MC --> HBM["HBM / GDDR Physical Memory"]
    end
```
