...
fn main() {
    println!("\n============================================================");
    println!("  621b. Task Scheduler with Multiple Machines");
    println!("============================================================");
    let tests: &[(&str, &[char], i32, usize, i32)] = &[
        ("LC621 ex1 m=1", &['A','A','A','B','B','B'], 2, 1, 8),
        ("LC621 ex2 m=1", &['A','C','A','B','D','B'], 1, 1, 6),
        ("LC621 ex3 m=1", &['A','A','A','B','B','B'], 3, 1, 10),
        ("fill idle slots m=1", &['A','A','A','B','B','B','C','C','D'], 2, 1, 9),
        ("single task type m=1", &['A','A'], 2, 1, 4),
        ("basic parallelism m=2", &['A','A','A','B','B','B'], 2, 2, 4),
        ("unique tasks m=2", &['A','C','A','B','D','B'], 1, 2, 3),
        ("large cooldown m=2", &['A','A','A','B','B','B'], 3, 2, 5),
        ("very many machines m=10", &['A','A','A','B','B','B'], 2, 10, 1),
        ("single task n=5 m=1", &['A'], 5, 1, 1),
        ("single task n=5 m=3", &['A'], 5, 3, 1),
        ("more machines than tasks n=0", &['A','B','C'], 0, 5, 1),
        ("n=0 same task m=2", &['A','A','A','A'], 0, 2, 2),
        ("m=3 n=1 four unique tasks", &['A','B','C','D'], 1, 3, 2),
        ("all same task m=2", &['A','A','A','A'], 1, 2, 3),
        ("all same task m=3", &['A','A','A','A','A','A'], 1, 3, 3),
        ("unique tasks no cooldown m=2", &['A','B','C','D','E'], 0, 2, 3),
    ];
    let mut passed = 0;
    for (i, (label, tasks, n, m, expected)) in tests.iter().enumerate() {
        let got = least_interval_multi(tasks, *n, *m);
        if got == *expected {
            passed += 1;
