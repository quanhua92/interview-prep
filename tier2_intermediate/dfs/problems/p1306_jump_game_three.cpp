...
int main() {
    ...
    struct { const char *label; std::vector<int> input; int start; bool expected; } tests[] = {
        {"example 1", {4, 2, 3, 0, 3, 1, 2}, 5, true},
        {"start at index 0", {4, 2, 3, 0, 3, 1, 2}, 0, true},
        {"cannot reach zero", {3, 0, 2, 1, 2}, 2, false},
        {"single element zero", {0}, 0, true},
        {"single element non-zero", {2}, 0, false},
        {"start next to zero", {0, 1}, 1, true},
        {"linear path to zero", {1, 1, 1, 1, 0}, 0, true},
        {"jump directly to zero", {7, 0, 0, 0, 0, 0, 0, 0}, 0, true},
    };
    ...
    for (int i = 0; i < n; i++) {
        bool got = canReach(tests[i].input, tests[i].start);
        if (got == tests[i].expected) {
            passed++;
            printf("  Test %d (%s): PASS\n", i + 1, tests[i].label);
        } else {
            printf("  Test %d (%s): FAIL\n", i + 1, tests[i].label);
            printf("    Expected: %d\n    Got:      %d\n", tests[i].expected, (int)got);
        }
    }
