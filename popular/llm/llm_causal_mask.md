...
```mermaid
graph TD
    subgraph 1. Query-Key Normalization
        Q["Raw Q: [B, L, H, D]"] --> QKN["RMSNorm(Q)"]
        K["Raw K: [B, L, H, D]"] --> KKN["RMSNorm(K)"]
    end
    subgraph 2. Shape Choreography & RoPE
        QKN --> RoPE_Q["Apply RoPE (L-axis)"]
        KKN --> RoPE_K["Apply RoPE (L-axis)"]
        RoPE_Q --> Trans_Q["Transpose: [B, H, L, D]"]
        RoPE_K --> Trans_K["Transpose: [B, H, S, D]"]
    end
    subgraph 3. Masked Dot Product
        Trans_Q & Trans_K --> Dot["Scores = (Q' @ K'^T) / √D<br/>[B, H, L, S]"]
        Mask["Mask[L, S]<br/>k = S - L"] --> Add["Scores = Scores + Mask"]
        Dot --> Add
        Add --> Softmax["probs = softmax(Scores, dim=-1)"]
        Softmax --> Out["Out = probs @ V"]
    end
```
