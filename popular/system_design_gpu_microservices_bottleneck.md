...
### System Architecture & Hardware Topologies (Mermaid)
...
graph TD
    ...
    %% KEDA & Queue
    subgraph QueueLayer ["Async Buffer & Scaling Layer"]
        Queue[(Redis Streams / Kafka)]
        KEDA[KEDA Autoscaler]
    end
    
    %% Compute Host
    subgraph Node ["GPU Compute Host (NUMA Node 0)"]
        CPU[CPU Socket 0]
        HostMem[Pinned Host CPU Memory]
        PCIe[PCIe Gen5 Switch]
        
        subgraph PhysicalGPU ["NVIDIA H100 Physical GPU"]
            subgraph MIG0 ["MIG Instance 0 (3g.40gb)"]
                subgraph MPS ["MPS Daemon"]
                    Worker1[OCR Worker Process 1]
                    Worker2[OCR Worker Process 2]
                end
            end
            subgraph MIG1 ["MIG Instance 1 (2g.20gb)"]
                ASR[Speech-to-Text Worker]
            end
        end
    end

    %% Connections
    Users -->|HTTPS / gRPC| Gateway
    Gateway -->|1. Enqueue Task| Queue
    Queue -->|2. Dequeue Task| Worker1
    Queue -->|2. Dequeue Task| ASR
    
    KEDA -->|Monitor Queue Depth| Queue
    KEDA -->|3. Scale Pods| Worker2

    %% Host Ingestion Pipeline
    CPU <-->|NUMA-Locality| HostMem
    HostMem -->|4. Pinned DMA over PCIe Gen5| PCIe
    PCIe -->|5. H2D Copy to VRAM| PhysicalGPU
    
    style Node fill:#ffe6cc,stroke:#d79b00,stroke-width:2px
    style PhysicalGPU fill:#dae8fc,stroke:#6c8ebf,stroke-width:2px
    style QueueLayer fill:#d5e8d4,stroke:#82b366,stroke-width:2px
```
