...
static int findPairs(const int *nums, int nums_size, int k) {
    ...
    memcpy(arr, nums, nums_size * sizeof(int));
    qsort(arr, nums_size, sizeof(int), cmp_int);
    int count = 0;
    int left = 0, right = 1;
    while (right < nums_size) {
        ...
        if (diff < k) {
            ...
            while (left < nums_size && arr[left] == lv) left++;
            while (right < nums_size && arr[right] == rv) right++;
        }
        if (left == right) right++;
    }
    free(arr);
