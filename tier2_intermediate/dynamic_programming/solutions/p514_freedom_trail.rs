...
fn find_rotate_steps(ring: &str, key: &str) -> i32 {
    ...
    for ki in (0..klen).rev() {
        let mut new_dp = vec![i32::MAX; n];
        let ch = (kb[ki] - b'a') as usize;
        for pos in 0..n {
            for &target in &positions[ch] {
                let diff = (pos as i32 - target as i32).abs();
                let steps = diff.min(n as i32 - diff) + 1;
                let val = if ki == klen - 1 { steps } else { steps + dp[target] };
