...
def parse_document_line(line):
    parts = line.split("|")
    vec = [float(x) for x in parts[0].split()]
    meta = {}
    if len(parts) > 1 and parts[1].strip():
        for kv in parts[1].strip().split():
            key, value = kv.split("=")
            meta[key] = value
    return vec, meta
...
def filter_and_search(query, documents, filters, k):
    m = len(filters)
    candidates = []
    for idx, (vec, meta) in enumerate(documents):
        passed = all(meta.get(key) == value for key, value in filters.items())
        if not passed:
            continue
        sim = sum(q * v for q, v in zip(query, vec))
        candidates.append((idx, sim, m))
