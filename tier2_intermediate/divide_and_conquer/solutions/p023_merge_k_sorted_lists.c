...
{
    if (left > right) { *out = NULL; *on = 0; return; }
    if (left == right) {
        *on = sizes[left];
        if (*on <= 0) { *out = NULL; *on = 0; return; }
        *out = malloc(*on * sizeof(int));
        memcpy(*out, lists[left], *on * sizeof(int));
        return;
    }
    int mid = left + (right - left) / 2;
    int *lh, *rh; int ln, rn;
    divide_merge(lists, sizes, left, mid, &lh, &ln);
    divide_merge(lists, sizes, mid + 1, right, &rh, &rn);
    merge_two(lh, ln, rh, rn, out, on);
    free(lh);
    free(rh);
}
