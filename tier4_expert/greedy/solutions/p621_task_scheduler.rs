...
fn main() {
    ...
    println!("============================================================");
    let tests: &[(&str, &[char], i32, i32)] = &[
        ("example 1", &['A','A','A','B','B','B'], 2, 8),
        ("example 2", &['A','C','A','B','D','B'], 1, 6),
        ("example 3", &['A','A','A','B','B','B'], 3, 10),
        ("tasks fill all idle slots", &['A','A','A','B','B','B','C','C','D'], 2, 9),
        ("single task type with long cooldown", &['A','A'], 2, 4),
        ("no cooldown", &['A','A','B','B'], 0, 4),
        ("single task", &['A'], 5, 1),
        ("all same task", &['A','A','A','A'], 2, 10),
        ("unique tasks large n", &['A','B','C'], 100, 3),
    ];
    let mut passed = 0;
    for (i, (label, tasks, n, expected)) in tests.iter().enumerate() {
        let got = least_interval(tasks, *n);
        if got == *expected {
            passed += 1;
