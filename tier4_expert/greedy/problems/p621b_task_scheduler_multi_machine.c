...
int main(void) {
    ...
    printf("============================================================\n");
    struct { const char *label; char tasks[16]; int tn; int n; int m; int expected; } tests[] = {
        {"LC621 ex1 m=1", "AAABBB", 6, 2, 1, 8},
        {"LC621 ex2 m=1", "ACABDB", 6, 1, 1, 6},
        {"LC621 ex3 m=1", "AAABBB", 6, 3, 1, 10},
        {"fill idle slots m=1", "AAABBBCCD", 9, 2, 1, 9},
        {"single task type m=1", "AA", 2, 2, 1, 4},
        {"basic parallelism m=2", "AAABBB", 6, 2, 2, 4},
        {"unique tasks m=2", "ACABDB", 6, 1, 2, 3},
        {"large cooldown m=2", "AAABBB", 6, 3, 2, 5},
        {"very many machines m=10", "AAABBB", 6, 2, 10, 1},
        {"single task n=5 m=1", "A", 1, 5, 1, 1},
        {"single task n=5 m=3", "A", 1, 5, 3, 1},
        {"more machines than tasks n=0", "ABC", 3, 0, 5, 1},
        {"n=0 same task m=2", "AAAA", 4, 0, 2, 2},
        {"m=3 n=1 four unique tasks", "ABCD", 4, 1, 3, 2},
        {"all same task m=2", "AAAA", 4, 1, 2, 3},
        {"all same task m=3", "AAAAAA", 6, 1, 3, 3},
        {"unique tasks no cooldown m=2", "ABCDE", 5, 0, 2, 3},
    };
    int tc = (int)(sizeof(tests) / sizeof(tests[0]));
    int passed = 0;
    for (int i = 0; i < tc; i++) {
        int got = leastIntervalMulti(tests[i].tasks, tests[i].tn, tests[i].n, tests[i].m);
